...
def create_scan_result(user_id, scan_data):
    ...
    """
    try:
        # Get access token from session to authenticate with RLS
        access_token = session.get('access_token')
        if not access_token:
        ...
        response = auth_supabase.table('scan_results').insert(db_data).execute()
...
def get_user_scan_results(user_id):
    ...
    try:
        ...
        response = auth_supabase.table('scan_results') \
            .select('*') \
